...
- `terraform/parameter`: Amazon SSM Parameter (plain text) --- AWS Resources in general, one or more.
...
### Note regarding Terraform Amazon IAM Long Term User Keys
You can provide keys from an Amazon User with permissions to only assume a role (and then configure the Terraform provider) to assume such role, thus scoping Terraform runners to their custom policies.
...
### Terraform IAM Roles and Amazon IAM Temporary Credentials
